    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.with_layout(Layout::left_to_right(egui::Align::Center), |ui| {
                ...
                let dest_rect = egui::Rect::from_min_size(ui.available_rect_before_wrap().min - Vec2::new(0.0, self.y_offset), ui.available_size());
                let height = ui.available_height();
                let rest_rect = egui::Rect::from_min_size(ui.available_rect_before_wrap().min + Vec2::new(0.0, dest_rect.max.y), ui.available_size());
                ...
                if forget {ctx.forget_all_images();}
                ui.painter().image(t.texture_id().unwrap(), dest_rect, uv, Color32::WHITE);
                if let Some(i) = self.svg[1].clone() {
                    if let Ok(t) = egui::Image::from_bytes(URI_2, i).load_for_size(ctx, rest_rect.size()){
                    
                        ui.painter().image(t.texture_id().unwrap(),rest_rect, uv, Color32::WHITE);
                    }
                }
            });
        });
...
impl MyApp {
    fn render_svg(&mut self) {
        self.world.update_file("main".to_string(), self.code.clone());
       if let Ok(d) = self.world.compile::<PagedDocument>().output {
            let current_pages: [Option<&Page>; 2] = [d.pages.get(self.current_page), d.pages.get(self.current_page + 1)];
